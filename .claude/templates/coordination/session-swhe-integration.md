...
1. `start-session` - Initializes session, updates SESSION.md
2. `session-start` - Creates SESSION.md structure with Current Focus
...
- All handlers hardcoded to SESSION.md
...
B) Create compatibility layer between SESSION.md and sessions/
...
- Secondary: Check SESSION.md (compatibility)
- Fallback: VOID→conventions
- Format: YYYY-MM-DD-NNN or YYYYMMDD or "current" or "VOID"
```

Also discovered:
- CLAUDE.md itself has NO direct SESSION.md references!
...
1. `.claude/templates/engine/execution/swhe-format.md` - S field spec
2. `.claude/templates/shared/patterns/ultrathink-format.md` - S field patterns
3. Handler files in `.claude/templates/handlers/*/session/*.md`
...
**Critical SESSION.md Dependencies:**
1. **ultrathink-format.md** (Line 22, 40, 97)
   - Defines S field as "Session ID from SESSION.md"
   - Validation checks for session existence
   - VOID fallback mechanism

2. **REGISTRY.md** (Multiple handlers)
   - `start-session` handler (Line 168): "updates SESSION.md"
   - `session-start` handler (Line 174): "Creates proper SESSION.md structure"
   - `update-session` handler (Line 178): "update SESSION.md"
   - File tracking section references SESSION.md for progress

3. **BEHAVIORS.md** (Critical session behaviors)
   - Line 40: S field checks date and SESSION.md
   - Line 160: Example of SESSION.md convention checks
   - Lines 312, 328, 343, 349, 356: Session management behaviors
   - Line 483: References SESSION.md Structure in CONVENTIONS

4. **Common Workflows** (serena-guide.md)
   - Lines 37, 43: "read SESSION.md" in activation patterns
   - Session continuation workflows depend on SESSION.md
...
- All write to SESSION.md directly
...
3. **Common Patterns**:
   ...
   - work tracking references to SESSION.md
...
Implementation:
  - SESSION.md becomes a view into sessions/current
  - Handlers continue using SESSION.md
...
Behavior:
  ...
  - Fall back to SESSION.md if not found
Pros:
...
1. Create SESSION.md ↔ sessions/current sync
2. Update handlers gradually
3. Eventually deprecate SESSION.md
...
5. Fallback → SESSION.md if it exists
...
1. Create `.claude/templates/engine/core/session-resolver.md`
2. Update swhe-format.md to use resolver
3. Keep SESSION.md as mirror of sessions/current
...
**Phase 3: Migration**
1. Convert existing SESSION.md to sessions/
2. Update all references gradually
3. Deprecate SESSION.md
...
## Purpose
Resolves session references from S field to actual session files, supporting both legacy SESSION.md and new sessions/ structure.
...
2. **Locate Session File**
   ...
   Try: SESSION.md (legacy fallback)
...
### SESSION.md Sync
- On write to sessions/current → update SESSION.md
- On read from SESSION.md → check sessions/ first
...
3. **Implementation Order:**
   ...
   3. Create SESSION.md ↔ sessions/current sync
...
2. Fall back to SESSION.md (legacy)
...
### Created Files:
1. ✓ `.claude/templates/engine/core/session-resolver.md` - Full resolver implementation

### Updated Files:
1. ✓ `.claude/templates/engine/execution/swhe-format.md` - Now uses session-resolver
2. ✓ `.claude/templates/shared/patterns/ultrathink-format.md` - Updated S field docs

### Key Changes Made:
- S field now supports multiple formats (current, YYYY-MM-DD-NNN, YYYYMMDD)
- Session resolver handles both sessions/ and SESSION.md
...
2. **Create sync mechanism** - SESSION.md ↔ sessions/current
...
### What We've Built:
A complete compatibility layer between old SESSION.md and new sessions/ directory:
