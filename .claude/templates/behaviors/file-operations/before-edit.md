...
2. Search for file-specific conventions:
   mcp__serena__search_for_pattern --substring_pattern "[filename] conventions" --relative_path ".claude/templates/CONVENTIONS.md"
...
## Example Workflow
```
TRIGGER: User requests "update SESSION.md with progress"
ACTION: 
1. Search: mcp__serena__search_for_pattern --substring_pattern "SESSION.md conventions" --relative_path ".claude/templates/CONVENTIONS.md"
2. Find: SESSION.md has specific timestamp format, progress log structure
3. Verify: Edit will follow the format
4. Proceed: Edit with correct conventions
```

## Common Files with Conventions
- **SESSION.md** - Timestamp format, progress structure, section order
...
## Cross-References
- [CONVENTIONS.md](../../templates/CONVENTIONS.md) - File-specific rules
