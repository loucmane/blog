...
export function ThemeSwitcher({
  ...
  useEffect(() => {
    ...
    const initialTimer = window.setTimeout(calculatePosition, 50)

    let resizeTimer: number | undefined
    const handleResize = () => {
      if (resizeTimer) window.clearTimeout(resizeTimer)
      resizeTimer = window.setTimeout(calculatePosition, 50)
    }
  ...
  const currentTheme =
    theme === 'system'
      ? { icon: '💻', label: 'System' }
      : (themes.find((t) => t.value === resolvedTheme) ?? themes[0] ?? fallbackTheme)
        ...
        {allThemes.map((themeOption, index) => {
          ...
          return (
            ...
            >
              ...
              {showLabels && (
                <span className="text-sm font-medium hidden lg:inline">{themeOption.label}</span>
              )}
            </button>
          )
        })}
