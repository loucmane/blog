...
export const semanticSpacing = {
  ...
  'nav-height': spacing[16],
  
  // Grid/Flex gaps
  'grid-gap-sm': spacing[2],
  'grid-gap-md': spacing[4],
  'grid-gap-lg': spacing[8],
} as const;

// Helper function to get negative spacing
export const negativeSpacing = Object.entries(spacing).reduce((acc, [key, value]) => {
  if (key !== '0') {
    acc[`-${key}`] = `-${value}`;
  }
  return acc;
