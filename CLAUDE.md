...
```
Once I find the handler in REGISTRY, I load it using:
- If exact path known: Read --file_path ".claude/templates/handlers/[role]/[domain]/[handler-name].md"
- If searching needed: mcp__serena__search_for_pattern --substring_pattern "id: handler-name" --relative_path ".claude/templates/handlers/"

Example: For `start-new-work` handler:
1. Registry shows: [start-new-work](handlers/triggers/development/start-new-work.md)
2. I load directly: Read --file_path ".claude/templates/handlers/triggers/development/start-new-work.md"
3. Or search if needed: mcp__serena__search_for_pattern --substring_pattern "id: start-new-work" --relative_path ".claude/templates/handlers/"
```
...
3. Load using one of:
   - Direct: Read --file_path ".claude/templates/handlers/triggers/development/start-new-work.md"
   - Search: mcp__serena__search_for_pattern --substring_pattern "id: start-new-work" --relative_path ".claude/templates/handlers/"
...
3. Load using one of:
   - Direct: Read --file_path ".claude/templates/handlers/triggers/debug/fix-bug.md"
   - Search: mcp__serena__search_for_pattern --substring_pattern "id: fix-bug" --relative_path ".claude/templates/handlers/"
...
3. Load using one of:
   - Direct: Read --file_path ".claude/templates/handlers/operators/git/commit-changes.md"
   - Search: mcp__serena__search_for_pattern --substring_pattern "id: commit-changes" --relative_path ".claude/templates/handlers/"
